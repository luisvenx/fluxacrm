...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, toggleSidebar, onNavigate, activeView }) => {
  ...
  return (
    <aside 
      className={`fixed top-0 left-0 bottom-0 z-50 w-72 bg-white border-r border-gray-200 shadow-2xl transition-transform duration-300 ease-in-out transform ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}
    >
      ...
      <div className="p-6 border-t border-gray-100 bg-gray-50/50">
        <div className="bg-white p-3 rounded-xl mb-6 flex items-center justify-between border border-gray-100 shadow-sm">
          <div className="flex flex-col">
            <span className="text-[9px] text-gray-400 uppercase font-black tracking-widest">Admin</span>
            <span className="text-xs text-gray-600 font-bold truncate max-w-[120px]">kyroossx@gmail.com</span>
          </div>
          <div className="flex items-center gap-1 bg-gray-50 p-1 rounded-lg border border-gray-100">
            <button className="p-1.5 hover:text-blue-600 text-gray-400 transition-colors"><Moon size={12} /></button>
            <button className="p-1.5 text-blue-600 bg-white rounded-md transition-colors shadow-sm"><Sun size={12} /></button>
          </div>
        </div>
        
        <div className="space-y-1">
          <button onClick={() => onNavigate('Usuários')} className="w-full flex items-center gap-3 p-3 text-gray-500 hover:text-gray-900 hover:bg-gray-100 rounded-xl transition-all text-sm font-bold">
            <Users size={18} />
            <span>Usuários</span>
          </button>
          <button onClick={() => onNavigate('Configurações')} className="w-full flex items-center gap-3 p-3 text-gray-500 hover:text-gray-900 hover:bg-gray-100 rounded-xl transition-all text-sm font-bold">
            <Settings size={18} />
            <span>Configurações</span>
          </button>
          <button className="w-full flex items-center gap-3 p-3 text-gray-500 hover:text-red-600 hover:bg-red-50 rounded-xl transition-all text-sm font-bold mt-2">
            <LogOut size={18} />
            <span>Sair</span>
          </button>
        </div>
