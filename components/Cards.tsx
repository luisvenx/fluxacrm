    ...
    <div className="bg-[#fcfcfd] min-h-screen space-y-8 animate-in fade-in duration-700 pb-20 px-6 lg:px-10 pt-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <div className="flex items-center gap-2 mb-1">
             ...
             <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Wallet Sincronizada via PostgreSQL</span>
          </div>
          <h2 className="text-3xl font-semibold text-slate-900 tracking-tight">Meus Cartões</h2>
          <p className="text-slate-500 font-medium">Gestão de limites e meios de pagamento corporativos.</p>
        </div>
        
        <button 
          onClick={() => setIsNewCardModalOpen(true)} 
          className="bg-blue-600 text-white px-6 py-2.5 rounded-full text-sm font-semibold hover:bg-blue-700 shadow-lg shadow-blue-600/20 flex items-center gap-2 transition-all active:scale-95"
        >
          <Plus size={18} />
          Vincular Cartão
        </button>
      ...
      {isLoading ? (
        <div className="py-32 text-center">
          ...
          <p className="text-xs font-bold text-slate-400 uppercase tracking-widest text-[10px]">Criptografando conexão...</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
          {cards.map(card => (
            <div key={card.id} className="relative group">
              <div className={`bg-white border border-slate-100 rounded-[2.5rem] p-8 shadow-sm hover:shadow-xl transition-all duration-500 overflow-hidden`}>
                ...
                <div className="flex justify-between items-start mb-10 relative z-10">
                  <div className={`w-14 h-10 ${card.color || 'bg-slate-900'} rounded-xl flex items-center justify-center text-white shadow-inner relative overflow-hidden group-hover:scale-110 transition-transform`}>
                    <div className="absolute top-2 left-2 w-3 h-2 bg-yellow-400/40 rounded-sm"></div> {/* Simulação de Chip */}
                    <CreditCard size={24} />
                  </div>
                ...
                <div className="space-y-6 relative z-10">
                  <div>
                    ...
                    <p className="text-xs text-slate-400 font-mono tracking-[0.25em] mt-1 italic">
                  ...
                  <div className="flex items-center gap-2 pt-2">
                    ...
                    <span className="text-[9px] font-black bg-emerald-50 text-emerald-600 px-3 py-1.5 rounded-full uppercase tracking-widest border border-emerald-100">
                      {card.status || 'Ativo'}
                    </span>
                  </div>
                </div>
          ...
          <button 
            onClick={() => setIsNewCardModalOpen(true)} 
            className="border-2 border-dashed border-slate-100 rounded-[2.5rem] p-8 flex flex-col items-center justify-center text-center gap-5 hover:bg-slate-50 hover:border-blue-100 transition-all group min-h-[320px] bg-white/50"
          >
            <div className="w-16 h-16 bg-white border border-slate-100 rounded-3xl flex items-center justify-center text-slate-300 group-hover:text-blue-600 shadow-sm transition-all group-hover:scale-110">
              <Plus size={32} />
            </div>
            <div>
      ...
      {!isLoading && cards.length > 0 && (
        <div className="flex items-center gap-3 bg-blue-50/50 border border-blue-100/50 p-6 rounded-[2rem] max-w-2xl">
          ...
          <div>
            ...
            <p className="text-xs text-blue-700/70 font-medium">Armazenamos apenas os últimos 4 dígitos para sua segurança. Todos os dados são criptografados em repouso no Supabase Vault.</p>
          </div>
        </div>
      )}
