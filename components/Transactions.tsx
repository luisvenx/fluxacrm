    ...
    <div className="bg-[#fcfcfd] min-h-screen space-y-8 animate-in fade-in duration-700 pb-20 px-6 lg:px-10 pt-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1">Dados reais persistidos no Supabase PostgreSQL.</p>
        </div>
        ...
        <div className="flex items-center gap-1 bg-slate-50 p-1 rounded-[1.25rem] w-full lg:w-auto">
          {tabs.map(tab => (
            <button
              ...
              className={`px-5 py-2 rounded-[1rem] text-xs font-bold transition-all ${selectedTab === tab ? 'bg-white text-slate-900 shadow-sm border border-slate-100' : 'text-slate-400'}`}
            >
              {tab}
            </button>
          ))}
              ...
              {isLoading ? (
                <tr>
                  <td colSpan={6} className="py-20 text-center">
                    ...
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Acessando Tabelas Supabase...</p>
                  </td>
                </tr>
              ) : transactions.length === 0 ? (
                <tr>
                  <td colSpan={6} className="py-20 text-center">
                    <p className="text-xs font-bold text-slate-300 uppercase tracking-[0.2em]">Nenhum registro encontrado</p>
                  </td>
                </tr>
              ) : (
                  ...
                  <tr key={item.id} className="hover:bg-slate-50/50 transition-all group cursor-pointer">
                    ...
                    <td className="px-8 py-5">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-[11px] text-slate-400 font-medium">{item.bank_accounts?.name || 'N/A'}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-8 py-5">
                      <span className="text-[10px] font-bold px-3 py-1 bg-slate-50 text-slate-500 rounded-full uppercase">
                    ...
                    <td className="px-8 py-5 text-center">
                      <span className={`text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest ${
                        item.status === 'PAID' ? 'bg-emerald-50 text-emerald-500 border border-emerald-100' : 
