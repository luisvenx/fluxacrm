...
const Marketing: React.FC<MarketingProps> = ({ user }) => {
  ...
  const stats = useMemo(() => {
    ...
    return [
      { label: 'Projetos', value: total.toString(), trend: 'Ativos', icon: <Globe size={18} />, color: 'blue' },
  ...
  if (isLoading) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center bg-[#fcfcfd] min-h-[80vh]">
        ...
        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-center px-4">Carregando marketing...</p>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        {stats.map((stat, i) => (
          <div key={i} className="bg-white border border-slate-100 rounded-[1.75rem] p-5 md:p-6 shadow-sm hover:shadow-md transition-all group">
            ...
            <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-2">{stat.trend}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
        <div className="lg:col-span-2 bg-white border border-slate-100 rounded-2xl md:rounded-[2.5rem] p-6 md:p-8 shadow-sm">
          <div className="flex items-center justify-between mb-8 md:mb-10">
            <div>
              <h3 className="text-base md:text-lg font-bold text-slate-900 tracking-tight">Fluxo de Produção</h3>
