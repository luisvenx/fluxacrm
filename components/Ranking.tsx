    ...
    <div className="bg-white border border-slate-100 rounded-[2.5rem] p-10 shadow-sm flex flex-col min-h-[450px] hover:shadow-2xl transition-all group relative overflow-hidden">
      <div className="flex items-center justify-between mb-10">
        <div className="flex items-center gap-4">
          <div className={`w-14 h-14 bg-slate-900 rounded-[1.5rem] flex items-center justify-center text-white border border-slate-700 shadow-lg group-hover:scale-110 transition-transform ${colorClass}`}>
            {icon}
          </div>
        ...
        {items.length === 0 ? (
          ...
          <div 
            ...
            className={`flex items-center justify-between p-5 rounded-2xl border transition-all ${
              idx === 0 ? 'bg-slate-900 border-slate-900 text-white shadow-xl scale-[1.03] z-10' : 'bg-slate-50/50 border-slate-100 text-slate-600 hover:bg-white hover:border-blue-100'
            }`}
          >
            <div className="flex items-center gap-4">
              <span className={`text-xs font-black w-4 ${idx === 0 ? 'text-blue-400' : 'text-slate-300'}`}>{idx + 1}</span>
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-[10px] font-black border ${
                idx === 0 ? 'bg-slate-800 border-slate-700 text-blue-400' : 'bg-white border-slate-200 text-slate-400'
              }`}>
                {item.initial}
              </div>
            ...
            <div className="flex items-center gap-3">
              <span className={`text-xl font-black tracking-tighter ${idx === 0 ? 'text-blue-400' : 'text-slate-900'}`}>{item.value}</span>
              {idx === 0 ? <Trophy size={16} className="text-amber-400 fill-amber-400" /> : <Medal size={16} className="text-slate-200" />}
            </div>
          </div>
        ))}
      </div>
      <div className="absolute bottom-0 left-0 h-1 w-0 group-hover:w-full transition-all duration-1000 bg-blue-600"></div>
    </div>
  );
  ...
  return (
    <div className="bg-[#fcfcfd] min-h-screen animate-in fade-in duration-700 pb-24 md:pb-20 px-6 md:px-10 pt-8 relative overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-12">
        <div>
          <h1 className="text-4xl font-black text-slate-900 tracking-tighter uppercase italic leading-none">
            Leaderboard <span className="text-blue-600 not-italic">Realtime</span>
          </h1>
        ...
        <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-2xl p-1 shadow-sm">
           {['Hoje', 'Este Mês', 'Geral'].map(range => (
             <button 
              ...
              className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${timeRange === range ? 'bg-blue-600 text-white shadow-md' : 'text-slate-400 hover:text-slate-900'}`}
             >
               {range}
             </button>
           ))}
      ...
      <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
        <RenderMetricCard title="Prospecções" icon={<PhoneCall size={22} />} items={rankings.prospeccoes} colorClass="group-hover:text-blue-400" />
        <RenderMetricCard title="Visitas" icon={<CalendarDays size={22} />} items={rankings.reunioes} colorClass="group-hover:text-indigo-400" />
        <RenderMetricCard title="Fechamentos" icon={<Trophy size={22} />} items={rankings.vendas} colorClass="group-hover:text-amber-400" />
      </div>

      {/* Your Stats Footer Area */}
      <div className="relative z-10 bg-[#203267] rounded-[3rem] p-12 text-white shadow-2xl overflow-hidden group">
          <div className="relative z-10 flex flex-col lg:flex-row items-center gap-12">
            <div className="relative shrink-0">
              <div className="w-32 h-32 bg-white/5 backdrop-blur-md text-blue-300 rounded-[2.5rem] flex items-center justify-center text-5xl font-black italic border border-white/10 shadow-inner group-hover:scale-105 transition-transform duration-700">
                {(user?.user_metadata?.full_name || 'U').substring(0,1).toUpperCase()}
              </div>
              <div className="absolute -bottom-2 -right-2 w-12 h-12 bg-blue-600 border-4 border-[#203267] rounded-2xl flex items-center justify-center text-white text-sm font-black shadow-xl italic">MVP</div>
            </div>
            ...
            <div className="flex-1 w-full space-y-10">
              <div>
                ...
                <div className="flex items-center gap-3 mt-2">
                   ...
                   <p className="text-[10px] font-black uppercase tracking-[0.4em] text-blue-400">Sua Performance Auditada SQL</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-12">
                 ...
                 <div className="flex items-center justify-end">
                    <button className="bg-white text-[#203267] px-8 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-400 hover:text-white transition-all shadow-xl active:scale-95 flex items-center gap-2">
