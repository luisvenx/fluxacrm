  ...
  return (
    <div className="bg-[#fcfcfd] min-h-screen flex flex-col animate-in fade-in duration-700 overflow-hidden pb-24 md:pb-10">
      <div className="px-4 md:px-8 pt-6 md:pt-8 pb-4 md:pb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto"><button onClick={fetchOnboardings} className="flex-1 md:flex-none p-2.5 bg-white border border-slate-200 text-slate-400 hover:text-blue-600 rounded-xl transition-all shadow-sm"><RefreshCcw size={18} className={isLoading ? 'animate-spin' : ''} /></button><button className="flex-[3] md:flex-none bg-blue-600 text-white px-6 py-2.5 rounded-xl md:rounded-full text-xs font-bold hover:bg-blue-700 transition-all flex items-center justify-center gap-2"><Plus size={20} /> Iniciar Fluxo</button></div>
      </div>

      <div className="px-4 md:px-8 pb-6 overflow-x-auto no-scrollbar"><div className="flex gap-4 md:grid md:grid-cols-4 min-w-max md:min-w-0">{stats.map((stat, i) => (<div key={i} className="bg-white border border-slate-100 rounded-2xl p-4 flex items-center justify-between shadow-sm min-w-[200px] md:min-w-0 group hover:border-blue-100 transition-all"><div className="space-y-0.5"><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{stat.label}</p><p className="text-lg font-black text-slate-900 tracking-tighter">{stat.value}</p><p className="text-[8px] font-bold text-blue-500 uppercase">{stat.trend}</p></div><div className={`p-2 bg-slate-50 text-slate-400 rounded-xl group-hover:scale-110 transition-transform`}>{React.cloneElement(stat.icon as React.ReactElement, { size: 18 })}</div></div>))}</div></div>
