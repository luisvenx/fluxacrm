      ...
      <div className="space-y-8">
        ...
        <div className="space-y-4">
          {isLoading ? (
            ...
            <div className="py-20 flex flex-col items-center justify-center text-center space-y-6 opacity-30"><div className="w-16 h-16 bg-slate-50 rounded-[2rem] flex items-center justify-center"><CalendarIcon size={32} className="text-slate-300" /></div><div className="max-w-[200px]"><p className="text-[10px] font-black uppercase tracking-widest leading-relaxed">Nenhum evento registrado</p></div></div>
          ) : (
            dayAppointments.map((item) => (
              <div key={item.id} className="bg-white border border-slate-100 p-6 rounded-[2rem] flex gap-5 items-center group hover:border-blue-200 hover:shadow-xl transition-all shadow-sm">
                ...
                <div className="flex-1 min-w-0">
                  <h4 className="text-sm font-bold text-slate-800 group-hover:text-slate-900 transition-colors truncate uppercase tracking-tight">{item.title}</h4>
                ...
                <CheckCircle2 size={20} className={`transition-colors cursor-pointer ${item.is_completed ? 'text-emerald-500' : 'text-slate-100 group-hover:text-slate-200'}`} />
              </div>
            ))
          )}
        </div>
